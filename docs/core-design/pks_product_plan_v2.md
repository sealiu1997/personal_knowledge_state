...
PKS 的 MCP Server 是 Kernel 的外部接入层，暴露意图级接口：

**只读（无需 token）：**
- `get_project_context` — 返回 PKS.md 内容
- `search_claims` — 按条件搜索 Claims
- `get_claim` — 获取单条 Claim
- `get_health` — 获取健康报告
- `list_projects` — 项目列表

**可写（需要 token）：**
- `submit_candidate_claim` — 提交候选 Claim（Kernel 校验 min_support）

Token 由 MCP 模块生成管理，权限校验在 Kernel。Agent 通过 MCP 与 PKS 交互，不直接访问 PKS 文件。

详细设计见 [`docs/adapter/mcp_design.md`](../adapter/mcp_design.md)。
...
| 参数 | 必须 | 说明 |
|------|------|------|
| `<project_id>` | ✅ | 位置参数，Capsule 稳定 ID |
| `--name` | ✅ | 人类可读项目名 |
| `--capsule-type` | ✅ | Capsule 类型（SoftwareCapsule/ArticleCapsule 等） |
| `--domain` | ✅ | 领域（dev/content/research） |
| `--stage` | ✅ | 当前阶段（转为初始 Claim） |
| `--current-goal` | 可选 | 当前目标（转为初始 Claim） |
| `--deliverable` | 可选 | 交付物（转为初始 Claim） |
| `--project-path` | 可选 | 外部项目路径 |
| `--git-remote` | 可选 | Git 仓库地址 |
| `--constraints` | 可选 | 逗号分隔的约束（转为 constraint Claim） |
| `--watched-paths` | 可选 | 逗号分隔的跟踪路径 |
| `--yes` | 可选 | 跳过确认 |

`stage`、`current-goal`、`deliverable`、`constraints` 是创建输入。Kernel 接受后生成初始 Claims，不作为长期知识字段留在 `project.yaml`。
...
pks new my-project \
  --name "..." --capsule-type SoftwareCapsule --domain dev \
  --stage "..." --current-goal "..." --project-path "..." --yes
```
...
- **P0-P1**：CLI 为主，验证核心闭环
- **P2**：Claim 审核工作台 + 项目仪表盘（最小 Web UI）✅ 已实现
- **P3**：完整管理界面（批量审核、Claim 新建/编辑、多维度浏览、证据链可视化、配置管理、MCP Token 展示）
...
```
PKS Kernel
├── Project Registry          # 项目注册与胶囊管理（组合：PolicyManager/TasteManager/ProjectionSpecManager/ClaimIdGenerator/ProjectSeeder）
├── Claim Workflow             # Claim 业务流程（submit/review/accept/reject/expire/supersede）
├── Claim Engine              # Claim 生命周期、min_support 校验、冲突检测
├── Health Engine             # stale 计算、evidence 检查、健康报告
├── Maintenance Engine        # 自动维护（stale 扫描、过期强制执行、evidence 重检查）
├── Review Engine             # accept/reject 候选流程
├── Review Strategy           # 可解释审核建议
├── Projection Service        # 投影编排（render/integrity/spec CRUD/edit API）
├── Projection Engine         # Claim 集合 → Markdown 渲染
├── Project Tracker           # 项目文件夹跟踪与变更检测
├── Candidate Queue           # 候选写入队列
├── Snapshot Manager          # 快照（复用 Git）
├── Audit Claim Factory       # 审计事件写成 inference Claim
└── Interface Adapters        # CLI（基础接口）/ Web UI / MCP（外部接入层）
```
...
| **P3** | Agent 协作 + 完整管理界面 | MCP Server · MCP Token 认证 · 完整 Web UI（批量审核、Claim 新建、多维度浏览、证据链可视化、配置管理） |
...
适配层设计文档：
- [MCP 适配层设计](../adapter/mcp_design.md)
- [Web UI 适配层设计](../adapter/web/overview.md)
