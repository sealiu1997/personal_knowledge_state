...
| 类型 | 含义 | 默认审核 |
|------|------|---------|
| `factual` | 事实性观察 | 低（P0/P1 给出 auto_accept 建议；后续可自动合并候选） |
...
| 级别 | 触发条件 | 处理方式 |
|------|----------|----------|
| **自动合并候选** | `type=factual` 且 confidence ≥ 阈值 且有 evidence | 后续阶段开放：策略驱动合并 + audit log |
...
- `0.3 ~ 0.5`：弱信号，不进入正式队列
- `0.5 ~ 阈值`：批量审核
- `≥ 阈值`（且满足自动合并条件）：P0/P1 给出 auto_accept 建议；后续阶段可自动合并
...
| 阶段 | 目标 | 核心交付 |
|------|------|----------|
| **P0** | Kernel 基线 | Capsule · Claim · Context Pack · `PKS.md` · health · tracking · audit · snapshot · CLI |
| **P1** | Claim 候选与审核闭环 | Candidate Queue · Review Engine · Review Strategy · `pks review` · Claim 查询筛选 · 领域策略校验 |
